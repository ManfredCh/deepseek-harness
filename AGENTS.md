...
Public APIs are pre-stable; update every consumer. Follow [version/status](docs/session-format-status.md), and [classify and record](docs/cookbook/reviewing-persistence-type-changes.md) declared persistence-type changes. [Adjacent migration](.agents/notes/implemented/architecture/2026-08-31-released-session-format-migrations.md) may add a version-named successor but never move, overwrite, or delete committed generations; predecessors imply neither fallback nor downgrade support. SQLite uses monotonic `SCHEMA_VERSION`.
