...
export function RightbarSeat({
  ...
  useEffect(() => {
    const pinned = tabTypes.filter(type=>type.pinned).map(type=>({kind:type.kind,contentId:pageAddress(type.kind),title:type.title(pageAddress(type.kind)),pinned:true,...type.keepMounted?{keepMounted:true}:{}}))
