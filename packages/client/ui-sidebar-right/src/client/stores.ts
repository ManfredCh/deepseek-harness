    ...
    actions: {
      ...
      openContent: (d, sessionId: string, intent, settled) => {
        d.bySession = seat(d, sessionId, s => advance(s, (state, mint) => {
          ...
          const index = intent.pinned ? 0 : lent === undefined || replace === undefined ? undefined : Math.max(lent.tabs.filter(id=>state.tabs[id]?.pinned).length,lent.tabs.indexOf(replace))
          ...
          const held = intent.pinned ? findContentTab(state,contentId,kind) : page ? panePage(state, paneId ?? activeDockPaneId(state), kind) : undefined
      ...
      duplicateTab: (d, sessionId: string, tabId: TabId) => {
        d.bySession = seat(d, sessionId, s =>
          advance(s, (state, mint) => state.tabs[tabId]?.pinned || pageKind(state, tabId) !== undefined ? [] : planDuplicateTab(state, mint, tabId).ops, seed))
      },
      ...
      placeTab: (d, sessionId: string, tabId: TabId, toPaneId: PaneId, index: number) => {
        d.bySession = seat(d, sessionId, s =>
          advance(s, state => state.tabs[tabId]?.pinned ? [] : arriving(state, tabId, toPaneId, () => planPlaceTab(state, tabId, toPaneId, Math.max(index,getPane(state,toPaneId).tabs.filter(id=>state.tabs[id]?.pinned).length))), seed))
      },
