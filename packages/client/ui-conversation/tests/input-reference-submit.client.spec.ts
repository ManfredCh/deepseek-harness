...
describe('reference submission', () => {
  it('publishes its own command effect after submitted text and attachments are consumed', async () => {
    ...
    expect(shell.beginCommand({ token: '/control', attachments: true, submit: async () => ({ kind: 'success', text: 'opaque-result' }) }, { start: 0, end: 8, draftRev: shell.snapshot.draftRev })).toBe(true)
    shell.submit()
