...
description: "Record a browser or Web UI interaction demo as an optimized GIF with the browser-control workflow and the bundled encoder, then attach it to a pull request. Use when asked to make, record, or generate a GIF that demonstrates a browser workflow, and for every pull request that changes product-user-visible GUI behavior."
---
