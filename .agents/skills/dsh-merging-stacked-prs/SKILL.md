...
description: "Land a stack of dependent GitHub PRs (A <= B <= C, each based on the one below) onto master through GitHub's native stacked-PR feature. Use when merging a PR whose base is another open PR's branch, when a request mentions stacked PRs, a PR stack, dependent PRs, or merging several related PRs in sequence."
---
